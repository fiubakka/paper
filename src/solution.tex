...
es el que hace uso de los \textit{behaviors} de Durable State provistos en el módulo de Akka Persistence. Esta decisión conlleva que al momento de iniciar el Player no dispongamos inmediatamente del estado
...
Se configura un \textit{timeout} de 30 segundos (cuyo valor es configurable) como límite de tiempo para recibir la respuesta. En caso de error desde el PlayerPersitor o que la respuesta no
...
lo consiga. En la práctica estos casos únicamente comienzan a suceder cuando el sistema está bajo una carga excesiva.
...
\noindent Se explicó en la sección de Cluster Sharding que las entidades de Akka son automáticamente terminadas cada cierta cantidad de tiempo en el que no reciban ningún mensaje.
...
no reciba el mensaje correspondiente, la terminación automática de Akka no entrará nunca en efecto. Esto se debe a que el GameEventConsumer continuará leyendo los distintos eventos que reciba desde Kafka, y seguirá enviándoselos
al Player. Dado que el Player nunca dejaría de recibir mensajes, la terminación nunca se activaría. En la práctica, esto se conoce coloquialmente como un \textit{leak} de recursos.
...
En nuestro caso, el PlayerHandler envía un mensaje de tipo \textbf{Heartbeat} hacia el Player cada 2 segundos. Cuando el Player recibe el mensaje actualiza un valor de su estado que indica el último \textit{timestamp}
en el que recibió el mensaje de Heartbeat. Adicionalmente, el Player se envía mensajes a sí mismo de forma periódica cada 5 segundos donde verifica si la diferencia entre el \textit{timestamp} actual y el almacenado en su estado
...
Si se activa la terminación del Player vía \textit{heartbeats}, se envía además un mensaje de tipo \textbf{Stop} al PlayerHandler para que cierre la conexión. En la práctica este mensaje se envía de forma preventiva, dado que lo más probable
...
\noindent Como vimos anteriormente, una de las ventajas de usar Akka Cluster Sharding es que permite el rebalanceo de entidades automáticamente al alterarse la composición de los nodos del Akka Cluster. Si bien es cierto que Akka se encargará de eliminar a una
entidad de un nodo e iniciarla nuevamente en otro, toda la lógica de sincronización de la capa de reglas de negocio es algo que debemos definir y manejar nosotros. Para el caso del Player, esto implica que al realizarse un reinicio de la entidad debemos 
...
El Player, en caso de recibir el mensaje de Heartbeat y estar en el \textit{behavior} de inicialización, almacenará la dirección del PlayerHandler en su estado y procederá a completar su inicialización. En este punto hemos recuperado el estado del Player
...
y comienza a simular movimientos periódicamente cada 16ms aproximadamente (60 movimientos por segundo), lo que en la práctica es la máxima carga permitida por un jugador real.
...
En nuestros análisis de performance no nos interesan tanto los tipos de mensajes generados sino la cantidad de actores y de mensajes que se estuvieran
...
\noindent En las anteriores secciones ahondamos en los distintos componentes que componen el proyecto \textit{Fiubakka} y las relaciones entre ellos. Partimos de los conceptos escenciales
de Akka y mostramos qué papel juega cada uno en nuestra aplicación. A modo de resumen y para tener una vista más panorámica de la arquitectura, el siguiente diagrama engloba
...
de la partida. A diferencia de la arquitectura \textit{core} de \textit{Fiubakka}, que vimos que es descentralizada, el estado del Truco se encuentra centralizado en una clase \textbf{TrucoMatch}, que es
...
Para garantizar la sincronización constante entre los clientes y el estado del Truco en el servidor decidimos utilizar un protocolo de \textit{acknowledgements} ante cada cambio de estado de la partida. A cada uno de estos cambios en el estado,
...
\noindent A continuación detallamos nuestra implementación del cliente utilizando las herramientas descritas
...
Cada nivel es una instancia de la clase \textbf{Level}, para que cada uno herede el comportamiento común
...
Para la recepción implementamos un ServerConsumer que se encarga de recibir los paquetes 
...
Esta solución sirvió en un inicio donde el juego era un prototipo donde no teníamos definidos 
