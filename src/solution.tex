...
Akka cuenta con varias herramientas para conseguir implementar la arquitectura mostrada previamente. Para poder entender los distintos componentes de nuestro servidor
...
El corazón de Akka es el modelo de actores. Es su principal componente y sobre el cual se construyen todos los demás.
...
Con todos los componentes de Akka presentados, podemos ahora proceder al detalle de nuestra implementación y los distintos componentes
...
Comenzamos entonces por el punto de entrada de conexión del cliente. El servidor está configurado para exponer un \textit{listener} de TCP en el puerto 2020.
Este listener es manejado por el actor \textbf{PlayerAccepter} mediante la creación de un Akka Stream que recibe como elementos
...
formado) tendremos un actor PlayerAccepter en cada nodo del juego, y los clientes podrían conectarse a cualquiera de los nodos para comenzar a jugar.

\begin{lstlisting}[language=Scala, caption={\textbf{Actor PlayerAccepter}}]
...
Cada conexión recibida en el servidor se procesa a través del mensaje \textbf{PlayerAccepter.Accept}.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{../assets/player-accepter.png}
    \caption{Flujo del actor PlayerAccepter}
\end{figure}
...
los mensajes. El protocolo de comunicación entre el cliente y el servidor será explciado más adelante, en este punto alcanza con entender
...
\subsubsection{Manejo del jugador del cliente}

Hasta aquí explicamos como se conecta un cliente al servidor y como se reciben los mensajes que el cliente emite.
...
Mencionamos en la introducción a la arquitectura de \textit{Fiubakka} que los eventos que deban ser notificados a todos los demás jugadores se realizan mediante mensajes de Kafka.
...
// TODO diagrama de flujo del gameevent consumer, codigo de ejemplo del producer y del consumer 
